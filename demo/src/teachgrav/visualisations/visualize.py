...
def _equal_aspect_limits(mins, maxs, buffer=1.0, figsize=None):
    ...
    y_half = max(y_range / 2.0 + buffer, (x_range / 2.0 + buffer) / aspect)
...
def axes(trajectory, options, figsize):
    ...
    if options == "trail":
        ...
        marker_sizes = marker_sizes_from_masses(
            trajectory.masses, fig_width_points
        )
        for i in range(num_bodies):
            (line,) = ax.plot(
                [], [], "o", color="lemonchiffon", markersize=marker_sizes[i]
            )
            lines.append(line)
    else:
...
def plot(trajectory, output, options, figsize):
    ...
    for i, line in enumerate(lines):
        line.set_data(*trajectory.positions()[:position, i, :].T)
    if output:
        plt.savefig(output)
    else:
        plt.show()
