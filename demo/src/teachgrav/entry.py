...
def _train_model(args, factory):
    ...
    logger.info(
        f"Training {args.law} model on {args.n_systems} "
...
def execute_scenario(args):
    ...
    factory = ScenarioFactory(
        args.engine, seed=args.seed, via_numpy=args.engine_consistent_seed
    )
...
def _validate_args(args):
    ...
    if args.outfile:
        logger.info(f"Selecting output format based on file extension: {
            args.outfile}")
...
def parse_args(force_args=None):
    ...
    parser.add_argument(
        "--method",
        default="euler",
        choices=["euler"] + diffrax_methods + scipy_methods,
    )
    parser.add_argument(
        "--law",
        default="gravity",
        choices=["gravity", "boids", "constant", "gaussian", "power"],
    ...
    parser.add_argument(
        "--log-file", default=None, help="File to save log output"
    )
    parser.add_argument(
        "--benchmark",
        action="store_true",
