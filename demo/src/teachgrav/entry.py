...
def entry():
    ...
    if args.visualise:
        ...
        if args.video: logger.info(f'video mode enabled')
        visualize(trajectory, output=args.outfile, mode='video' if args.video else 'plot', options=args.visualise)
    else:
        logger.info(f"Outputting trajectory data to {args.outfile if args.outfile else 'stdout'}")
...
def parse_args(force_args=None):
    ...
    parser.add_argument('--outfile', default=None, help='Output file for visualization (e.g. .mp4 or .gif)')
    parser.add_argument('--visualise', default='trail', choices=['trail', 'dot'], help='Visualization style')
    parser.add_argument('--loglevel', default='WARNING', help='Logging level (e.g. DEBUG, INFO, WARNING)')
    parser.add_argument('--log-file', default=None, help='File to save log output')
    parser.add_argument('--video', action='store_true', help='Whether to create a video output (implies --visualise)')
    parser.add_argument('--format', default=None, choices=['csv', 'mp4', 'png'], 
                        help='Output format for trajectory data (e.g. csv, json, png). Inferred from outfile extension if not specified.')
    args = parser.parse_args(force_args.split() if force_args else None)
    if args.outfile and not args.format:
        logger.info(f"Selecting output format based on file extension: {args.outfile}")
        if args.outfile.endswith('.mp4'):
            ...
            logger.warning(f"Unknown file extension for output: {args.outfile}. Defaulting to stdout text.")
            args.visualise = None
            args.format = 'csv'
