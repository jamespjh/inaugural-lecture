...
def law(system: System) -> np.ndarray:
    ...
    # Each body experiences a gravitational force from every other body, leading to acceleration
    # So we have an N*N*2 matrix of pairwise position differences
    # ... and thus an N*N*2 matrix of pairwise accelerations
    # Which we sum over the second axis to get the total acceleration on each body
    ...
    accelerations = -1.0*G * masses[np.newaxis, :, np.newaxis] * displacements / (distances ** 3)
    ...
    return np.stack([d_positions, d_velocities]) # Shape (2 (pos, vel), N, 2 (x and y), )
