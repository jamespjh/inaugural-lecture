...
_METHOD_ENGINE_COMBOS = [
    ...
    for m, e in [
        ("RK45", "numpy"),
        ("LSODA", "numpy"),
        #    ('Tsit5', 'jax-cpu'), TODO: JAX integration needs
        # factory propagated, needs tidy up
    ]
...
def test_close_to_start_after_one_orbit(method, engine):
    ...
    assert start_pos.__array_namespace__().allclose(
        start_pos, end_pos, atol=0.02
    )
...
_jax_single_body_cases = (
    [("jax-cpu", "Tsit5", 1.0, 1e-6)] if "jax-cpu" in AVAILABLE_ENGINES else []
)
