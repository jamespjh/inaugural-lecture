...
def test_convergence_video_rejected_for_gaussian():
    ...
    with pytest.raises(
        ValueError,
        match="Convergence video output is only " "supported for --law power.",
    ):
        parse_args(
            "--train --law gaussian --scenario scatter "
            "--outfile /tmp/conv.mp4"
        )
...
def test_convergence_video_with_checkpoint_interval():
    ...
    checkpoints = [
        {"G": float(i) * 0.1, "power": 2.0 + i * 0.1} for i in range(10)
    ]
...
def test_convergence_video_downsamples_frames_to_duration_times_fps():
    ...
    checkpoints = [{"G": 1.0 + i, "power": 2.0 + i} for i in range(6)]
